...
def startTerminal(net, hostname):
    host = net.getNodeByName(hostname)
    return host.popen("xterm", shell=True)
...
def main():
    ...
    net = Mininet(topo=SimpleTopo(), switch=Router, controller = OVSController)
    net.start()
    ...
    startWebserver(net, 'h31', "Default web server")
    startWebserver(net, 'h41', "*** Attacker web server ***")
