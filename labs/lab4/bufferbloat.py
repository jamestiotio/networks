...
parser.add_argument(
    "--cong", dest="cong", help="Congestion control algorithm to use", default="reno"
)
parser.add_argument(
    "--diff",
...
class StarTopo(Topo):
    ...
    def __init__(
        self, n=2, cpu=None, bw_host=1000, bw_net=1.5, delay="10", maxq=None, diff=False
    ):
...
def bbnet():
    ...
    ping_latency(net)
    print(
        "Initially, the delay between two hosts is around %dms" % (int(args.delay) * 2)
    )
    h2 = net.getNodeByName("h2")
    h1 = net.getNodeByName("h1")
    h1.cmd("cd ./http/; nohup python3 ./webserver.py &")
    h1.cmd("cd ../")
